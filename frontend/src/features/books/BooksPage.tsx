...
import { getBooks, getBooksStats } from '../../services/books';
...
export function BooksPage() {
  ...
  const [createAuthorModalOpened, { open: openCreateAuthorModal, close: closeCreateAuthorModal }] =
    useDisclosure(false);
  ...
  const { data: booksStats, isLoading: isBooksStatsLoading } = useQuery({
    queryKey: ['books', 'stats'],
    queryFn: getBooksStats,
  });

  const { createBookMutation, updateBookMutation, deleteBookMutation, updateStatusMutation } =
    useBookMutations();

  const { createAuthorMutation } = useAuthorMutations();
  ...
  function handleStatusFilterChange(value: ReadingStatus | null) {
    setStatus(value);
    setPage(1);
  }

  function handleAuthorFilterChange(value: number | null) {
    setAuthorId(value);
    setPage(1);
  }
    ...
    <>
      <Container size="xl" py="xl">
        ...
        <BookFormModal
          ...
          authors={authors}
          genres={genres}
          onSubmit={(data) => createBookMutation.mutateAsync(data)}
        ...
        <CreateAuthorModal
          ...
          onSubmit={(data) => createAuthorMutation.mutateAsync(data)}
