...
import type { FavoriteGenre } from '../../../types/dashboard';
...
export function FavoriteGenresCard({ genres }: FavoriteGenresCardProps) {
  ...
  const lastColor = hasGenres
    ? genreColors[(genres.length - 1) % genreColors.length].color
    : firstColor;
  ...
  const gradientSections: string[] = [`${seamColor} 0%`, `${firstColor} ${seamSize}%`];

  genres.forEach((genre, index) => {
    const percentage = total > 0 ? (genre.count / total) * 100 : 0;

    const start = accumulatedPercentage;
    const end = start + percentage;

    const currentColor = genreColors[index % genreColors.length].color;

    const nextColor = genreColors[(index + 1) % genreColors.length].color;

    const transitionSize = Math.min(3, percentage / 4);

    const transitionStart = end - transitionSize;

    if (index === 0) {
      gradientSections.push(`${currentColor} ${Math.max(start, seamSize)}%`);
    } else {
      gradientSections.push(`${currentColor} ${start}%`);
    }

    if (index === genres.length - 1) {
      gradientSections.push(
        `${currentColor} ${Math.min(transitionStart, 100 - seamSize)}%`,
        `${seamColor} 100%`,
      );
    } else {
      gradientSections.push(`${currentColor} ${transitionStart}%`, `${nextColor} ${end}%`);
    }
  ...
  return (
    ...
    >
      <Stack gap="xl">
        <Group gap="sm">
          <IconChartDonut size={21} color="var(--bookshelf-text-muted)" />
        ...
        {!hasGenres ? (
          <Center mih={240}>
            <Stack gap="xs" align="center">
              <IconChartDonut size={42} stroke={1.3} color="var(--bookshelf-text-muted)" />

              <Text fw={500}>{t('dashboard.favoriteGenres.emptyTitle')}</Text>

              <Text size="sm" className="bookshelf-text-muted" ta="center">
                {t('dashboard.favoriteGenres.emptyDescription')}
              </Text>
            </Stack>
          </Center>
        ) : (
          <Group justify="center" align="center" gap={48} mih={240}>
            <div
            ...
            <Stack gap="md">
              {genres.map((genre, index) => {
                const percentage = total > 0 ? Math.round((genre.count / total) * 100) : 0;

                return (
                  <Group key={genre.id} gap="sm" wrap="nowrap">
                    <span
                      style={{
                        width: 10,
                        height: 10,
                        borderRadius: '50%',
                        flexShrink: 0,
                        background: genreColors[index % genreColors.length].dot,
                      }}
                    />

                    <Text size="sm">
                      {genre.name}{' '}
                      <Text component="span" className="bookshelf-text-muted" size="sm">
                        ({percentage}%)
                      </Text>
                    </Text>
                  </Group>
